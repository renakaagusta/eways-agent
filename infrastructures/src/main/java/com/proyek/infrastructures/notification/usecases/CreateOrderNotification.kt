...
class CreateOrderNotification: ViewModel() {
    private val services = NotificationApiServices::class.java
    private val result = MutableLiveData<ArrayList<NotificationResponse>>()

    fun set(recevierId: String, orderId: String,title: String, description: String, context: Context) {
        MyRetrofit
            .createService(services)
            .createOrderNotification(recevierId, orderId,title, description)
