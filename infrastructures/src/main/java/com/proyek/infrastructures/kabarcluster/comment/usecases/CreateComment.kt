...
class CreateComment: ViewModel() {
    private val services = CommentApiServices::class.java
    private val result = MutableLiveData<CommentResponse>()

    fun set(userId: String, postId: String, postContent: String, context: Context) {

        MyRetrofit
            .createService(services)
            .createComment(userId, postId, postContent)
            .enqueue(object : Callback<CommentResponse?> {
                override fun onFailure(call: Call<CommentResponse?>?, t: Throwable?) {
                    Log.d("commenterror", t?.message)
