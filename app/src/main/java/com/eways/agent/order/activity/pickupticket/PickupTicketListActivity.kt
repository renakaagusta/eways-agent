...
class PickupTicketListActivity : BaseActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CustomSupportActionBar.setCustomActionBar(this, "Pickup Ticket")
    ...
    override fun onStart() {
        ...
        getAgentCreatedOrder = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(GetAgentCreatedOrder::class.java)

        user = intent.getParcelableExtra("user")

        this@PickupTicketListActivity.showProgress()

        getAgentCreatedOrder.set(user.ID!!,2, this@PickupTicketListActivity)
        getAgentCreatedOrder.get().observe(this, Observer {
            ...
            listData.reverse()
            setPickupTicketData(listData)

            this@PickupTicketListActivity.dismissProgress()
        })
    }
    ...
    private fun setPickupTicketData(listOrderViewDTO: ArrayList<OrderBasicViewDTO>){
        val listPickupTicketViewDTO : MutableList<IOrderViewDTO> = arrayListOf()
        for (orderViewDTO in listOrderViewDTO) {
            if(orderViewDTO.orderType == OrderType.PickupTicket && orderViewDTO.orderStatus == OrderStatus.Created){
                listPickupTicketViewDTO.add(orderViewDTO)
            }
        }

        if (listPickupTicketViewDTO.isEmpty()){
            apply {
            ...
            val historyOrderAdapter = OrderListAdapter(listPickupTicketViewDTO, user, listOrder)
            rvItem.apply {
                isVisible =true
                layoutManager = LinearLayoutManager(this@PickupTicketListActivity)
