...
class LayananBebasListActivity :BaseActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CustomSupportActionBar.setCustomActionBar(this, "LayananBebas")
    ...
    override fun onStart() {
        ...
        getAgentCreatedOrder = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(
            GetAgentCreatedOrder::class.java)

        user = intent.getParcelableExtra("user")

        this@LayananBebasListActivity.showProgress()

        getAgentCreatedOrder.set(user.ID!!,7, this@LayananBebasListActivity)
        getAgentCreatedOrder.get().observe(this, Observer {
            ...
            listData.reverse()
            setLayananBebasData(listData)

            this@LayananBebasListActivity.dismissProgress()
        })
    }
    ...
    //{"name": "Jvjv", "service": {"id": "1apSvFBkIq0ScrIeQWX8cGl3T8X", "name": "Layanan Bebas", "type": 7, "agent_fee": 10000, "created_at": "2020-04-21 08:07:50", "updated_at": "2020-04-21 08:07:50", "description": "Customer menyediakan detail layanan yang ingin dilakukan", "service_fee": 20000}, "description": "Jvig"}
    private fun setLayananBebasData(listOrderViewDTO: ArrayList<OrderBasicViewDTO>){
        val listLayananBebasViewDTO : MutableList<IOrderViewDTO> = arrayListOf()
//1bLDdsZuRo4kSjZ8yA2y516Ai0k
        for (orderViewDTO in listOrderViewDTO) {
            if(orderViewDTO.orderType == OrderType.LayananBebas && orderViewDTO.orderStatus == OrderStatus.Created){
                Log.d("masuk", orderViewDTO.toString())
                listLayananBebasViewDTO.add(orderViewDTO)
            }
        }

        if (listLayananBebasViewDTO.isEmpty()){
            apply {
            ...
            val historyOrderAdapter = OrderListAdapter(listLayananBebasViewDTO, user, listOrder)
            rvItem.apply {
                isVisible =true
                layoutManager = LinearLayoutManager(this@LayananBebasListActivity)
