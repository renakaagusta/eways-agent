...
class GantiPaketListActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CustomSupportActionBar.setCustomActionBar(this, "Ganti Paket")
    ...
    override fun onStart() {
        ...
        this@GantiPaketListActivity.showProgress()

        getAgentCreatedOrder.set(user.ID!!,3, this@GantiPaketListActivity)
        getAgentCreatedOrder.get().observe(this, Observer {
            ...
            listData.reverse()
            setGantiPaketData(listData)

            this@GantiPaketListActivity.dismissProgress()
        })
    }
    ...
    private fun setGantiPaketData(listOrderViewDTO: ArrayList<OrderBasicViewDTO>){
        val listGantiPaketViewDTO : MutableList<IOrderViewDTO> = arrayListOf()
        for (orderViewDTO in listOrderViewDTO) {
            if(orderViewDTO.orderType == OrderType.GantiPaket && orderViewDTO.orderStatus == OrderStatus.Created){
                listGantiPaketViewDTO.add(orderViewDTO)
            }
        }

        if (listGantiPaketViewDTO.isEmpty()){
            apply {
            ...
            val historyOrderAdapter = OrderListAdapter(listGantiPaketViewDTO, user, listOrder)
            rvItem.apply {
                isVisible =true
                layoutManager = LinearLayoutManager(this@GantiPaketListActivity)
