...
class SOPPListActivity : BaseActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        override fun onStart() {
            ...
            this@SOPPListActivity.showProgress()

            getAgentCreatedOrder.set(user.ID!!, 5, this@SOPPListActivity)
            getAgentCreatedOrder.get().observe(this, Observer {
                ...
                listData.reverse()
                setSOPPData(listData)

                this@SOPPListActivity.dismissProgress()
            })
        }
        ...
        private fun setSOPPData(listOrderViewDTO: ArrayList<OrderBasicViewDTO>) {
            val listSOPPViewDTO: MutableList<IOrderViewDTO> = arrayListOf()
            for (orderViewDTO in listOrderViewDTO) {
                if (orderViewDTO.orderType == OrderType.SOPP && orderViewDTO.orderStatus == OrderStatus.Created) {
                    listSOPPViewDTO.add(orderViewDTO)
                }
            }

            if (listSOPPViewDTO.isEmpty()) {
                apply {
                ...
                val historyOrderAdapter = OrderListAdapter(listSOPPViewDTO, user, listOrder)
                rvItem.apply {
                    isVisible = true
                    layoutManager = LinearLayoutManager(this@SOPPListActivity)
