...
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.*
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.imgProfile
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.tvAccept
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.tvCustomerAddress
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.tvCustomerCluster
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.tvCustomerName
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.tvCustomerPhone
import kotlinx.android.synthetic.main.activity_layananbebas_onrequest.tvDate
...
class LayananBebasOnRequestActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_layananbebas_onrequest)
    ...
    override fun onStart() {
        ...
        getCustomerDetail = ViewModelProvider(
            this,
            ViewModelProvider.NewInstanceFactory()
        ).get(GetCustomerDetail::class.java)
        acceptOrder = ViewModelProvider(
            this,
            ViewModelProvider.NewInstanceFactory()
        ).get(AcceptOrder::class.java)
        createNotification = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(
            CreateOrderNotification::class.java
        )
    ...
    private fun setData(lifecycleOwner: LifecycleOwner) {
        GlobalScope.launch(Dispatchers.Main) {
            ...
            this@LayananBebasOnRequestActivity.showProgress()

            getOrderDetail.set(this@LayananBebasOnRequestActivity, orderId)
            delay(500)
            ...
            getCustomerDetail.set(customerId, this@LayananBebasOnRequestActivity)
            delay(1000)
            getCustomerDetail.get().observe(lifecycleOwner, Observer {
                ...
                if (it.data[0].imagePath != null)
                    Glide.with(this@LayananBebasOnRequestActivity)
            ...
            this@LayananBebasOnRequestActivity.dismissProgress()

        }
    }

    fun moveToMainActivity() {
        val intent = Intent(this@LayananBebasOnRequestActivity, MainActivity::class.java)
    ...
    private fun acceptOrder() {
        tvAccept.setOnClickListener {
            this@LayananBebasOnRequestActivity.showProgress()
            acceptOrder.set(orderId, this@LayananBebasOnRequestActivity)
            acceptOrder.get().observe(this, Observer {if(it.errors.message?.isEmpty()!!) {
                GlobalScope.launch(Dispatchers.Main) {
                    createNotification.set(
                        ...
                        orderId,
                        user.username!!,
                        "Order Layanan Bebas mu telah dikonfirmasi oleh Agen",
                        this@LayananBebasOnRequestActivity
                    )
                    delay(300)
                    createNotification.set(
                        user.ID!!,
                    ...
                    setConfirm(this@LayananBebasOnRequestActivity::moveToMainActivity)
                }
            } else {
                this@LayananBebasOnRequestActivity.dismissProgress()
                this@LayananBebasOnRequestActivity.showError(it.errors.message!![0])
            }
            })
        }
    }
