...
class PSBListActivity : BaseActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CustomSupportActionBar.setCustomActionBar(this, "PSB")
    ...
    override fun onStart() {
        ...
        this@PSBListActivity.showProgress()

        getAgentCreatedOrder.set(user.ID!!,1, this@PSBListActivity)
        getAgentCreatedOrder.get().observe(this, Observer {
            ...
            this@PSBListActivity.dismissProgress()
        })
    }
    ...
    private fun setPSBData(listOrderViewDTO: ArrayList<OrderBasicViewDTO>){
        val listPSBViewDTO : MutableList<IOrderViewDTO> = arrayListOf()
        for (orderViewDTO in listOrderViewDTO) {
            if(orderViewDTO.orderType == OrderType.PSB && orderViewDTO.orderStatus == OrderStatus.Created){
                listPSBViewDTO.add(orderViewDTO)
            }
        }

        if (listPSBViewDTO.isEmpty()){
            apply {
            ...
            val historyOrderAdapter = OrderListAdapter(listPSBViewDTO, user, listOrder)
            rvItem.apply {
                isVisible =true
                layoutManager = LinearLayoutManager(this@PSBListActivity)
